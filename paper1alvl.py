...
class Company:
    ...
    def GetBalanceTrack(self):
        return self._BalanceTrack
    ...
    def GetDetails(self):
        ...
        Details += "Current balance: " + f"{self._Balance:.2f}" + "\nAverage cost per meal: " + str(
            self._AvgCostPerMeal) + "\n"
        Details += "Balance track: " + str(self._BalanceTrack) + "\n"
    ...
    def ProcessDayEnd(self):
        ...
        if len(self._Outlets) > 1:
            Distance = self.CalculateMinDeliveryDistance()
            DeliveryCosts = self._BaseCostOfDelivery + self.CalculateDeliveryCost(Distance)
        else:
        ...
        self._BalanceTrack.append(float(f"{self._Balance:.2f}"))
        Details += "New balance for company: " + f"{self._Balance:.2f}" + "\n"
        Details += "Balance track over time: " + str(self._BalanceTrack)
        return Details
    ...
    def CalculateDeliveryDistance(self, ListOfOutlets):
        TotalDistance = 0.0
    ...
    def CalculateMinDeliveryDistance(self):
        listOfOutlets = self.GetListOfOutlets()
        permtations = itertools.permutations(listOfOutlets)
        minDistance = 1000000000000000000000000
        for i in permtations:
            distance = self.CalculateDeliveryDistance(i)
            if distance < minDistance:
                minDistance = distance
        return minDistance

    def CalculateFastDeliveryDisatnce(self):
        listOfOutlets = self.GetListOfOutlets()
        nearestList = []
        totalDistance = 0
        currentOutlet = listOfOutlets[0]
        nearestList.append(currentOutlet)
        listOfOutlets.remove(currentOutlet)
        while len(listOfOutlets) != 0:
            min = -1
            for i in listOfOutlets:
                distance = self.__GetDistanceBetweenTwoOutlets(currentOutlet, i)
...
class Simulation:
    ...
    def GetDaysPast(self):
        return self._DaysPast
    ...
    def DisplayMenu(self):
        ...
        print("6. Advance a number of days")
        print("7. Display delivery distances")
        print("8. Display settlement map")
        print("Q. Quit")
        print("\nEnter your choice: ", end="")

    def DisplaySettlementMap(self):
        size = (self._SimulationSettlement.GetXSize(), self._SimulationSettlement.GetYSize())
        map = Image.new("RGB", size, (255, 255, 255))
        map_draw = ImageDraw.Draw(map)
        for household in self._SimulationSettlement.get_households():
            coords = [household.GetX(), household.GetY()]
            map_draw.ellipse((coords[0]-5, coords[1]-5, coords[0]+5, coords[1]+5), fill="black")

        company_colours = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0), (255, 0, 255), (0, 255, 255)]
        for i in range(len(self._Companies)):
            for outlet in self._Companies[i]._Outlets:
                coords = outlet.GetX(), outlet.GetY()
            ...
            if EatsOut:
                ...
                while Current < len(Reputations):
                    if CompanyRNo < Reputations[Current]:
                        ...
                        break
                    Current += 1
        self.CheckForBankruptcy()
        self.__DisplayCompaniesAtDayEnd()
        self.__DisplayEventsAtDayEnd()
        self._DaysPast += 1

    def CheckForBankruptcy(self):
        for c in self._Companies:
            balance = c.GetBalance()
            if balance <= 0:
                self._Companies.remove(c)
    ...
    def DisplayDeliveryDistances(self):
        for company in self._Companies:
            min_distance = company.CalculateMinDeliveryDistance()
            fast_distance = company.CalculateFastDeliveryDisatnce()
            print(f"Min delivery distance of {company.GetName()} is {min_distance}, fast delivery distance is {fast_distance}")
    ...
    def Run(self):
        ...
        while Choice != "Q":
            ...
            if Choice == "1":
                ...
                NoDays = int(input("Enter number of days: "))
                for i in range(NoDays):
                    self.ProcessDayEnd()
            elif Choice == "7":
                self.DisplayDeliveryDistances()
            elif Choice == "8":
                self.DisplaySettlementMap()
            elif Choice == "Q":
