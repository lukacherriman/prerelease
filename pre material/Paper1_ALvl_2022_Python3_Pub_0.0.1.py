...
class Breakthrough():
        ...
        if len(self.__Locks) > 0:
            ...
            while not self.__GameOver:
                ...
                while not self.__LockSolved and not self.__GameOver:
                    ...
                    print(f"there are {self.__Deck.GetNumberOfCards()} cards")
    ...
    def __Peek(self):
        Cards = [self.__Deck.GetCardDescriptionAt(0), self.__Deck.GetCardDescriptionAt(1), self.__Deck.GetCardDescriptionAt(2)]
        self.__PeekUsed = True
        return Cards

    def __Muligan(self):
        DeckCards = [self.__Deck.GetCardDescriptionAt(card) for card in range(self.__Deck.GetNumberOfCards())]
        HandCards = [self.__Hand.GetCardDescriptionAt(card) for card in range(self.__Hand.GetNumberOfCards())]
        DiscardCards = [self.__Discard.GetCardDescriptionAt(card) for card in range(self.__Discard.GetNumberOfCards())]
        SequenceCards = [self.__Sequence.GetCardDescriptionAt(card) for card in range(self.__Sequence.GetNumberOfCards())]
        AllCards = DeckCards + HandCards + DiscardCards + SequenceCards
        ...
        for i in range(len(AllCards)):
            NewCard = ToolCard(AllCards[i][0], AllCards[i][2])
    ...
    def __GetChoice(self):
        print()
        if self.__PeekUsed and self.__MuliganUsed:
            Choice = input("(D)iscard inspect, (U)se card:> ").upper()
        elif self.__PeekUsed:
            Choice = input("(D)iscard inspect, (U)se card, (M)uligan:> ").upper()
        elif self.__MuliganUsed:
            Choice = input("(D)iscard inspect, (U)se card, (P)eek:> ").upper()
        else:
            Choice = input("(D)iscard inspect, (U)se card, (M)uligan, (P)eek:> ").upper()
        return Choice
