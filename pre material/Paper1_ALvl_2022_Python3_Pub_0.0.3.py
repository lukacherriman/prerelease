...
class Breakthrough():
    ...
    def __PlayCardToSequence(self, CardChoice):
        description = self.__Hand.GetCardDescriptionAt(CardChoice - 1)
        if description[2] == "m":
            set = input("Enter which set you would like the card to be of: ")
            self.__Hand.SetCardToolKit(CardChoice, set)
    ...
    def __CheckIfLockChallengeMet(self):
        ...
        for Count in range(self.__Sequence.GetNumberOfCards() - 1, max(0, self.__Sequence.GetNumberOfCards() - 3) -1, -1):
            ...
            if self.__CurrentLock.CheckIfConditionMet(SequenceAsString):
                if self.__Sequence.GetNumberOfCards() - (Count) == 3:
                    print("yes")
                    for i in range(self.__Sequence.GetNumberOfCards() - 1, Count-1, -1):
                        print(i)
                        self.__MoveCard(self.__Sequence, self.__Discard, self.__Sequence.GetCardNumberAt(i))
                return True
        return False
...
class CardCollection():
    ...
    def SetCardToolKit(self, CardChoice, set):
        self._Cards[CardChoice-1].SetCardToolKit(set)
