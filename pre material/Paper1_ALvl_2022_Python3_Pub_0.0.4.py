...
class Breakthrough():
    ...
    def __SaveGame(self):
        file_number = input("Enter the number of the game file to be saved to: ")
        with open(f'game{file_number}.txt', 'w') as writer:
            score = self.__Score
            writer.write(f"{score}\n")
            challenges = self.__CurrentLock.ChallengeToString()
            print(challenges)
            new = ""
            for i in range(len(challenges)):
                for j in range(len(challenges[i][0])):
                    new += challenges[i][0][j]
                    if j < len(challenges[i][0])-1:
                        new += ','
            ...
            for i in range(self.__Hand.GetNumberOfCards()):
                new += f"{self.__Hand.GetCardDescriptionAt(i)} {self.__Hand.GetCardNumberAt(i)}"
                if i < self.__Hand.GetNumberOfCards()-1:
                    new += ','
            writer.write(f"{new}\n")
            new = ''
            for i in range(self.__Sequence.GetNumberOfCards()):
                new += f"{self.__Sequence.GetCardDescriptionAt(i)} {self.__Sequence.GetCardNumberAt(i)}"
                if i < self.__Sequence.GetNumberOfCards() - 1:
                    new += ','
            writer.write(f"{new}\n")
            new = ''
            for i in range(self.__Discard.GetNumberOfCards()):
                new += f"{self.__Discard.GetCardDescriptionAt(i)} {self.__Discard.GetCardNumberAt(i)}"
                if i < self.__Discard.GetNumberOfCards() - 1:
                    new += ','
            writer.write(f"{new}\n")
            new = ''
            for i in range(self.__Deck.GetNumberOfCards()):
                new += f"{self.__Deck.GetCardDescriptionAt(i)} {self.__Deck.GetCardNumberAt(i)}"
                if i < self.__Deck.GetNumberOfCards() - 1:
                    new += ','
            writer.write(f"{new}\n")
        writer.close()

    def __LoadGame(self, FileName):
        file_number = input("Enter the number of the game file: ")
        try:
            with open(f"game{file_number}.txt", 'r') as f:
    ...
    def __GetChoice(self):
        print()
        Choice = input("(D)iscard inspect, (S)ave, (U)se card:> ").upper()
        return Choice
...
class Lock():
    ...
    def ChallengeToString(self):
        challenges = [[challenge.GetCondition(), challenge.GetMet()] for challenge in self._Challenges]
        return challenges
